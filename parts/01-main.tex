...
\begin{itemize}
  ...
  \item That means: $(a \cdot x) \mod n = 1$
...
$A \mod B$ denotes the \textbf{remainder} of the division of $A$ by $B$.


\subsection{RSA Cryptography}
\begin{itemize}
  \item The plaintext $M$ is converted into a ciphertext $C$: $C = M^{e} \mod S$
  \item $S$ and $e$ are the \textbf{public keys}, i.e., anyone can use them to encrypt their messages
  \item The plaintext $M$ for a ciphertext $C$ is then recovered: $M = C^{d} \mod S$
  \item Where $d$ is the \textbf{private key}, i.e., it is secret, and only the recipient knows it
  \item For RSA to work $M = (M^{e})^{d} \mod S$ must hold for all positive integers $M < S$
  \item This holds if:
  \begin{itemize}
    ...
    \item $d$ is a \textbf{positive inverse} of $e \mod (p-1)(q-1)$, i.e., $ed \equiv 1 \pmod{(p-1)(q-1)}$
  \end{itemize}
